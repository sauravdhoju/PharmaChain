...
const MedicineInventory: React.FC = () => {
    ...
    return medicines
      ...
      .sort((a, b) => {
        if (sortBy === SortBy.Sales) {
          return b.patientSales.totalSales - a.patientSales.totalSales;
  ...
  return (
    <Box p={6} maxW="1120px" margin="0 auto" overflowY="auto">
        ...
        <HStack spacing={4} alignItems="center">
          ...
          <HStack>
            ...
            <Switch
              ...
              onChange={(e) => setViewMode(e.target.checked ? ViewMode.Card : ViewMode.Table)}
            />
            <Text>Card View</Text>
          </HStack>
        </HStack>
      ...
      <Box mb={6} role="alert">
        {filteredMedicines.some((medicine) => medicine.expiry.daysLeft <= 0) && (
          <HStack
            p={4}
            bg="red.50"
            borderRadius="lg"
          ...
          >
            ...
            <Text color="red.500" fontWeight="medium">
              Some medicines are past their expiry date. Immediate action required!
            </Text>
          </HStack>
        )}
      </Box>
            ...
            <Tbody>
              {filteredMedicines.map((medicine) => (
                <Tr >
                  ...
                  <Td>
                    <Text fontWeight="medium">{medicine.quantity.stock}</Text>
                  </Td>
                  <Td>
                    <HStack>
                      <CalendarIcon
                        color={medicine.expiry.daysLeft <= 0 ? "red.500" : "yellow.500"}
                      />
                      <Badge
                        colorScheme={medicine.expiry.daysLeft <= 0 ? "red" : "yellow"}
                      >
                        {medicine.expiry.daysLeft} days left
                      </Badge>
                    </HStack>
                    <Text fontSize="sm">
                      Recommended Period: {medicine.expiry.recommendedPeriod} days
                    </Text>
                  </Td>
                  <Td>
                    <VStack align="start">
                        <Text fontWeight="bold">Total Sales</Text>
                      <Text>Total Sales: {medicine.patientSales.totalSales} units</Text>
                        <Text fontWeight="bold">Peer Sales</Text>
                        <Text>Build Sales Potential: {medicine.patientSales.totalSales} units</Text>
                      <Progress
                        colorScheme="green"
                        size="sm"
                        value={(medicine.patientSales.totalSales / 1000) * 100}
                      />
                    </VStack>
                  </Td>
                  <Td>
                    <Button
                      ...
                      onClick={() => handlePeerOpportunitiesClick(medicine.name)}
                    >
                      Peer Opportunities
                    </Button>
                  </Td>
            ...
            <Card >
              <CardBody>
                <VStack align="start" spacing={3}>
                  ...
                  <Text>Total Sales: {medicine.patientSales.totalSales} units</Text>
                  <Text fontWeight="bold">Peer Sales</Text>
                  <Text>Buld Sales Potential: {medicine.patientSales.totalSales} units</Text>
                  <Progress
                    colorScheme="green"
                    size="sm"
                    value={(medicine.patientSales.totalSales / 1000) * 100}
